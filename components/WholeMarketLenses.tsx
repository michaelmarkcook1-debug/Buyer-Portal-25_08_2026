...
export function WholeMarketLenses({ vendors }: { vendors: VendorIntel[] }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {calls.map((c, i) => (
        <Panel key={c.ticker} className="px-5 py-4">
          <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
            <span className="code text-[0.78rem]" style={{ color: "var(--accent-ink)" }}>
              #{i + 1}
            </span>
            <Link
              href={`/vendors/${c.ticker.toLowerCase()}`}
              className="text-[1.02rem] font-medium underline-offset-4 hover:underline"
              style={{ color: "var(--fg)", textDecorationColor: "var(--accent-fill)" }}
            >
              {c.name}
            </Link>
            <span className="text-[0.92rem]" style={{ color: "var(--fg-muted)" }}>
              — {c.headline}
            </span>
          </div>
          <p className="m-0 mt-2 text-[0.86rem] leading-relaxed" style={{ color: "var(--fg-muted)" }}>
            <span style={{ color: "var(--fg-dim)" }}>Why: </span>
            {c.why}
          </p>
          <p className="m-0 mt-1 text-[0.86rem] leading-relaxed" style={{ color: "var(--fg)" }}>
      ...
      {tiedNote ? (
        <p className="m-0 text-[0.8rem]" style={{ color: "var(--fg-dim)" }}>
          {tiedNote}
        </p>
      ) : null}
    </div>
